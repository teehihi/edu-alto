  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <span
          className={cn(
            "inline-flex h-12 w-12 items-center justify-center rounded-xl p-2.5",
            style.iconBox,
          )}
        >
          <Image
      ...
      <p
        className={cn(
          "mt-6 min-h-[72px] text-sm leading-7",
          feature.tone === "primary" ? "text-white/90" : "text-muted",
        )}
      >
        {feature.description}
      </p>
      <a
        href="#courses"
        className={cn(
          "focus-ring mt-auto inline-flex items-center gap-2 rounded-md text-sm font-semibold transition",
          style.link,
        )}
      >
        Tìm hiểu thêm
