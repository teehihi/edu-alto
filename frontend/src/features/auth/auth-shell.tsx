  ...
  return (
    <div className="relative min-h-screen bg-white lg:h-screen lg:overflow-hidden">
      ...
      >
        <Image
          src={
            isLeft ? "/images/auth/login-background.png" : "/images/auth/register-background.png"
          }
          alt={panelAlt}
          fill
          priority
      ...
      >
        <div className="mx-auto w-full max-w-[660px]">
          ...
          <h1 className="mb-4 text-center text-[24px] font-semibold leading-[1.3] tracking-normal text-primary sm:mb-5 sm:text-[28px]">
            {title}
          </h1>
          {children}
        </div>
    ...
    <div className="space-y-2.5">
      ...
      {provider ? (
        <p
          id={statusId}
          role="status"
          className="rounded-xl bg-footer p-2.5 text-xs leading-5 text-muted sm:text-sm"
        >
...
export function OtpInput({ value, onChange, disabled, error, autoFocus = true }: OtpInputProps) {
  ...
  return (
    <div className="w-full space-y-2.5">
      <div
        className={cn(
          "relative mx-auto h-14 w-full max-w-[420px] sm:h-16",
          disabled && "opacity-60",
        )}
      >
        ...
        />
        <div
          className="pointer-events-none absolute inset-0 grid grid-cols-6 gap-2.5 sm:gap-3.5"
          aria-hidden="true"
        >
            ...
            return (
              <span
                ...
                className={cn(
                  ...
                  error ? "border-red-400" : isActive ? "border-primary" : "border-[#DADDE3]",
                )}
              >
                {hasChar ? (
                  value[index]
      ...
      {error ? (
        <p id={`${id}-error`} className="text-center text-xs font-medium text-red-600 sm:text-sm">
          {error}
        </p>
      ) : null}
    </div>
...
export function OtpSuccessModal({ open, seconds = 3 }: { open: boolean; seconds?: number }) {
  ...
  return (
    ...
    >
      <Image
        className="mx-auto h-[160px] w-[160px] object-contain sm:h-[185px] sm:w-[190px]"
        src="/images/auth/confirmation.png"
        alt=""
        width={190}
        height={185}
      />
      <h2
        ...
        className="mt-2 text-[26px] font-bold leading-tight text-primary sm:text-[32px]"
      >
        Email đã được xác thực
      </h2>
      <p
        id="otp-success-description"
        className="mx-auto mt-2.5 max-w-[280px] text-xs leading-5 text-[#1A535C] sm:text-sm"
      >
        Bạn sẽ được chuyển đến trang đăng nhập sau {seconds} giây nữa!
      </p>
      <Link
        href="/login"
