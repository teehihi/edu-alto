...
describe("ContactPage", () => {
  it("renders contact header, form, contact details and map", () => {
    ...
    expect(screen.getByRole("heading", { name: /kết nối với edualto/i, level: 1 })).toBeInTheDocument();
    expect(screen.getByText("Để lại lời nhắn cho chúng tôi")).toBeInTheDocument();
    expect(screen.getByText("HCM-UTE, 01 Võ Văn Ngân, phường Linh Chiểu, thành phố Thủ Đức, TP. Hồ Chí Minh")).toBeInTheDocument();
  ...
  it("submits the contact form successfully and shows confirmation modal", async () => {
    ...
    await user.type(screen.getByPlaceholderText(/nhập nội dung thắc mắc/i), "Tôi muốn tìm hiểu khóa học Spring Boot.");
